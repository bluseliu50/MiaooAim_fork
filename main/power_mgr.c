 ...
 * GPIO46 不能参与 EXT1，掩码里若含 46 会导致 esp_sleep_enable_ext1_wakeup
 * 失败、按键全不能唤醒。 SW4 仍可在运行时使用；深睡后请用 SW3 或 SW5
 * 唤醒，或等定时唤醒。
 */
...
esp_err_t power_mgr_init(void) {
  ...
  ESP_LOGI(TAG, "Config: enabled=%d, interval=%d min, idle=%d s", s_cfg.enabled,
           s_cfg.interval_min, s_cfg.idle_timeout_s);

  return ESP_OK;
}

bool power_mgr_is_timer_wake(void) { return s_timer_wake && s_cfg.enabled; }
...
esp_err_t power_mgr_set_config(const power_config_t *cfg) {
  ...
  if (validated.interval_min < 1)
    validated.interval_min = 1;
  if (validated.interval_min > 1440)
    validated.interval_min = 1440;
  if (validated.idle_timeout_s < 30)
    validated.idle_timeout_s = 30;
  if (validated.idle_timeout_s > 600)
    validated.idle_timeout_s = 600;
  portENTER_CRITICAL(&s_activity_mux);
  s_cfg = validated;
  portEXIT_CRITICAL(&s_activity_mux);
...
esp_err_t power_mgr_arm(void) {
  ...
  s_last_activity_us = esp_timer_get_time();
  portEXIT_CRITICAL(&s_activity_mux);
  ESP_LOGI(TAG, "Armed: will sleep after %d s of inactivity",
           s_cfg.idle_timeout_s);
  return ESP_OK;
}
...
static void enter_sleep_internal(bool from_idle, int64_t idle_snapshot_us) {
  ...
  if (from_idle && activity_after(sleep_request_us)) {
    ESP_LOGI(TAG,
             "Sleep postponed: display/user activity while waiting for EPD");
    return;
  }
  ...
  if (last_epd_us > 0) {
    ...
    if (elapsed_ms < EPD_SLEEP_SETTLE_MS) {
      ...
      ESP_LOGI(TAG, "Waiting %d ms after EPD refresh before deep sleep",
               wait_ms);
  ...
  uint64_t gpio_mask = (1ULL << WAKE_GPIO_SW3) | (1ULL << WAKE_GPIO_SW5);
  esp_err_t werr =
      esp_sleep_enable_ext1_wakeup(gpio_mask, ESP_EXT1_WAKEUP_ANY_HIGH);
  /* 数字域下拉(pull_down_en)在 deep sleep 会随数字 GPIO 域断电而失效，
   * 引脚进入高阻浮空；配合 EXT1 ANY_HIGH 会立即误触发唤醒 → 全量重启。
   * 这里显式启用 RTC 下拉、关闭 RTC 上拉，把电平锁死为低，只在按键真正
   * 拉高时才唤醒。gpio_num_t 与 rtc_gpio_num_t 等价，直接转换即可。 */
  rtc_gpio_pullup_dis((gpio_num_t)WAKE_GPIO_SW3);
  rtc_gpio_pulldown_en((gpio_num_t)WAKE_GPIO_SW3);
  rtc_gpio_pullup_dis((gpio_num_t)WAKE_GPIO_SW5);
  rtc_gpio_pulldown_en((gpio_num_t)WAKE_GPIO_SW5);
  if (werr != ESP_OK)
    ESP_LOGE(TAG, "ext1 wakeup config failed: %s (check RTC GPIO mask)",
             esp_err_to_name(werr));

  ESP_LOGI(TAG,
           "Entering deep sleep for %llu s (configured interval=%d min, EXT1: "
           "GPIO %d/%d only; SW4 not on RTC)",
...
void power_mgr_enter_sleep(void) { enter_sleep_internal(false, 0); }
