...
/* Schematic: SW3->GPIO9(prev), SW4->GPIO46(refresh), SW5->GPIO3(next), 10k
 * pulldown. */
...
static void show_unconfigured_hint(int idx, unsigned epoch) {
  ...
  ui_draw_empty_state(fb, "\xe6\x9c\xaa\xe9\x85\x8d\xe7\xbd\xae",
                      "\xe8\xaf\xb7\xe9\x80\x9a\xe8\xbf\x87\xe7\xbd\x91\xe9\xa1"
                      "\xb5\xe8\xae\xbe\xe7\xbd\xae");
...
static void switch_mode(int dir) {
  ...
  /* Skip disabled modes using the user-configured cycle order. */
  s_mode_idx = (dir >= 0) ? display_mode_next_enabled(s_mode_idx)
                          : display_mode_prev_enabled(s_mode_idx);
...
static void on_press(int idx) {
  ...
  case 0:
    cmd = CMD_PREV;
    ESP_LOGI(TAG, "SW3 (prev)");
    break;
  case 1:
    cmd = CMD_REFRESH;
    ESP_LOGI(TAG, "SW4 (refresh)");
    break;
  case 2:
    cmd = CMD_NEXT;
    ESP_LOGI(TAG, "SW5 (next)");
    break;
  default:
    return;
  }
...
esp_err_t button_init(void) {
  ...
  ok = xTaskCreate(display_worker_task, "btn_disp", WORKER_STACK, NULL, 4,
                   &worker_h);
  if (ok != pdPASS) {
  ...
  ESP_LOGI(TAG, "3 buttons on GPIO %d/%d/%d, %d modes registered", BTN_LEFT,
           BTN_MID, BTN_RIGHT, display_mode_count());
  return ESP_OK;
}
